...
class FasterKANLayer(nn.Module):
    ...
    def forward(self, x, time_benchmark=False):
        ...
        if not time_benchmark:
            spline_basis = self.rbf(self.layernorm(x)).view(x.shape[0], x.shape[1]*x.shape[2], -1)
            # print("spline_basis:", spline_basis.shape)
        else:
            spline_basis = self.rbf(x).view(x.shape[0], x.shape[1]*x.shape[2], -1)
            # print("spline_basis:", spline_basis.shape)
        # print("-------------------------")
        # ret = 0
        ret = self.spline_linear(spline_basis).view(x.shape[0], x.shape[1], x.shape[2], -1)
